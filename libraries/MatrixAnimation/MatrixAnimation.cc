...
uint8_t MatrixAnimation::random_state() {
	auto pix_state = new bool[_xmax][_ymax];
...
uint8_t MatrixAnimation::rand_color_init() {
  ...
  for (int i = 0; i < _xmax/4; i++) {
    ...
    _col_sz[i] = _matrix->Color444(r, g, b);
    //_col_sz[i] = _matrix.Color444((uint8_t)random(2,8), (uint8_t)random(10,14), (uint8_t)random(6,10));
  }
}
...
uint8_t MatrixAnimation::uramp(int16_t x, int16_t y) {
    ...
    _matrix->drawPixel(x, y, _matrix->Color444(i, i, i));
    delay(20);
  }
...
uint8_t MatrixAnimation::spectrum(uint8_t bits, int del) {
  ...
  if (bits == 3) {
    for (int i = 1; i < 8; i++) {
      red = 7 - i;
      green = i;
      _matrix->fillScreen(_matrix->Color333(red, green, blue));
      delay(del);
    }
    for (int i = 1; i < 8; i++) {
      green = 7 - i;
  ...
  if (bits == 4) {
    for (int i = 1; i < 16; i++) {
      red = 15 - i;
      green = i;
      _matrix->fillScreen(_matrix->Color444(red, green, blue));
      delay(del);
    }
    for (int i = 1; i < 16; i++) {
      green = 15 - i;
  ...
  if (bits == 8) {
    for (int i = 1; i < 256; i++) {
      red = 255 - i;
      green = i;
      _matrix->fillScreen(_matrix->Color888(red, green, blue));
      delay(del);
    }
    for (int i = 1; i < 256; i++) {
      green = 255 - i;
  ...
  switch (quadrant) {
    ...
    case 0:
      for (int z = _xmin; z < _xmax - width; z++) {
        ...
        //_matrix->fillScreen(_matrix->Color333(0, 7, 7));
        for (int w = 0; w < width; w++) {
          _matrix->drawLine(z + w, _ymin, z + w, _ymax, color);
          _matrix->drawLine(_xmin, z + w, _xmax, z + w, color);
        }
        delay(del);
        // To clear the lines, otherwise they stay on screen
        for (int w = 0; w < width; w++) {
          _matrix->drawLine(z + w, _ymin, z + w, _ymax, _matrix->Color333(0, 0, 0));
          _matrix->drawLine(_xmin, z + w, _xmax, z + w, _matrix->Color333(0, 0, 0));
        }
      }
      break;
    // (_xmax, 0)
    case 1:
      for (int z = _xmin; z < _xmax - width; z++) {
        for (int w = 0; w < width; w++) {
          _matrix->drawLine(_xmax - (z + w), _ymin, _xmax - (z + w), _ymax, color);
          _matrix->drawLine(_xmin, z + w, _xmax, z + w, color);
        }
        delay(del);
        // To clear the lines, otherwise they stay on screen
        for (int w = 0; w < width; w++) {
          _matrix->drawLine(_xmax - (z + w), _ymin, _xmax  - (z + w), _ymax, _matrix->Color333(0, 0, 0));
          _matrix->drawLine(_xmin, z + w, _xmax, z + w, _matrix->Color333(0, 0, 0));
        }
      }
      break;
    ...
    case 2:
      for (int z = _xmin; z < _xmax + width; z++) {
        for (int w = 0; w < width; w++) {
          _matrix->drawLine(_xmax - (z + w), _ymin,  _xmax - (z + w), _ymax, color);
          _matrix->drawLine(_xmin, _ymax - (z + w), _xmax,  _ymax - (z + w), color);
        }
        delay(del);
        // To clear the lines, otherwise they stay on screen
        for (int w = 0; w < width; w++) {
          _matrix->drawLine(_xmax - (z + w), _ymin, _xmax - (z + w), _ymax, _matrix->Color333(0, 0, 0));
          _matrix->drawLine(_xmin, _ymax - (z + w), _xmax, _ymax - (z + w), _matrix->Color333(0, 0, 0));
        }
      }
      break;
    // (0, _ymax)
...
uint8_t MatrixAnimation::square_zoom(int del) {
  ...
  for (int16_t i = 1; i <= _xmax / 2; i++) {
    _matrix->drawRect(_xmax / 2 - i , _ymax / 2 - i, i * 2, i * 2, _col_sz[j]);
    if (i % 2 == 0) {
      j++;
    }
  }
  ...
  _col_sz[0] = _matrix->Color444(r, g, b);
  //_col_sz[0] = _matrix->Color444((uint8_t)random(2,8), (uint8_t)random(10,14), (uint8_t)random(6,10));
...
uint8_t MatrixAnimation::zoom_2col(int del = 100) {
	...
	for (int16_t j = 0; j < 2; j++) {
		for (int16_t i = 1; i <= _xmax / 2; i++) {
			...
			if (j == 0) {
				if ( i % 4 == 0 || i % 4 == 1) {
					_matrix->drawRect(_xmax / 2 - i, _ymax / 2 - i, i * 2, i * 2, _col_2c[0]);
				}
				else {
					_matrix->drawRect(_xmax / 2 - i, _ymax / 2 - i, i * 2, i * 2, _col_2c[1]);
				}
			}
			else {
				if ( i % 4 == 2 || i % 4 == 3) {
