...
uint8_t nearestNeighbor(uint8_t led, uint8_t *nn, boolean continuous, boolean square) {
  ...
  if (!continuous) {
    ...
    // by just sorting in ascending order, since 255 is the largest
    uint8_t _tmp, i;
    int8_t j;
    for (i = 1; i < 6; i++) {
      _tmp = nn[i];
...
void setRingColor(uint8_t ring, CRGB col, boolean height) {
